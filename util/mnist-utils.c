...
MNIST_Image getImage(FILE *imageFile, int train_test){
    ...
    // //fp1 = fopen("image_pixels_1", "a");
    // fp_training = fopen("image_pixels_training", "a");
    // fp_testing = fopen("image_pixels_testing", "a");
    //char buffer[16];
    // if(train_test == 0)
    // {
    //     for (int i = 0; i < sizeof(img); i++)
    //     {   
    //         (i % 28 == 0) ? (fprintf(fp_training, "%s", img.pixel[i] > 1 ? "X\n" : ".\n") ) : (fprintf(fp_training, "%s", img.pixel[i] > 1 ? "X" : ".") );
    //     }
    //     fprintf(fp_training,"%s", "\n");
    //     fclose(fp_training);
    // }
    ...
    //     for (int i = 0; i < sizeof(img); i++)
    //     {  
    //         (i % 28 == 0) ? (fprintf(fp_testing, "%s", img.pixel[i] > 1 ? "X\n" : ".\n") ) : (fprintf(fp_testing, "%s", img.pixel[i] > 1 ? "X" : ".") );
    //     }
    //     fprintf(fp_testing,"%s", "\n");
    //     fclose(fp_testing);
    // }
